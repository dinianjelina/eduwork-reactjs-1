...
const Hook = () => {
  let [keyword, setKeyword] = useState('tesla');
  ...
  useEffect(() => {
    ...
    const fetchNews = async () => {
      try {
        const response = await fetch(`https://newsapi.org/v2/everything?q=${keyword}&from=2022-03-22&sortBy=publishedAt&apiKey=${process.env.REACT_APP_API}`);
